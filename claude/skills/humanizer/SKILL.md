...
description: |
  ...
  Use when the user says humanize, unslop, run the humanizer, make it sound human, or
  asks "does this sound AI", and as the mandatory final gate on any prose deliverable
  (LinkedIn post, article, landing copy, CV, email, Jira or PR text). Catches not-X-but-Y
  contrasts, one-line closers, staged openers, forced triads, dashes, inflated claims,
  sales language, stock AI words, bold labels, filler, colon connectors, abstract
  metaphor nouns, feeling-words instead of mechanisms, and adverb propping. Based on
  Wikipedia's "Signs of AI writing" plus house rules.
license: MIT
version: 3.1.1-house
metadata:
  upstream: https://github.com/blader/humanizer (SKILL.md on main, version 3.0.0, fetched 2026-09-13)
  house_source: harness repo, claude/skills/humanizer/SKILL.md. Edit there, then run install.py.
  bump_rule: no version bump without both cases under evals/ passing in a fresh session.
---
...
Sections 1 to 25 are upstream 3.0.0 verbatim in substance. The House section adds five patterns and three overrides that win over anything above them. The Quick pass section is the short numbered list for in-flight cleanups (the former unslop skill).
...
Two rules follow from this. Every sentence you keep must add something the reader did not already have. A tell counts in proportion to how rarely a careful writer would make it on purpose. The patterns are numbered strongest first: sections 1 to 5 justify an edit on one sighting, and a pattern marked *weak alone* needs company from other tells in the same passage before you act.
...
3. **Check the draft.** Read it aloud. Ask what still sounds AI-generated. Ask whether the rewrite added or dropped any fact, name, number, date, quote, citation, ranking, or claim that things happen at once; shape edits under sections 6, 9, and 19 drop those most often. Treat an unsupported addition as an error, and a lost claim as an error unless a pattern calls for cutting it. Then search for the five tells that most often survive a rewrite: a not-X-but-Y contrast, a one-line closer, a dash, a triad, a bold label.
...
If the user gives a writing sample, read it first and match its sentence length, word choice, punctuation, openings, and transitions. The sample overrides the patterns below, with one exception: the House override on dashes wins over the sample.

Without a sample, take the voice from the kind of text. Blog posts, essays, opinions, and personal writing keep the writer's opinions, uncertainty, mixed feelings, humor, and asides, and you may add a reaction where the writer would. Reference, technical, legal, and factual text stays neutral and plain. Removing tells is half the job; the result must still sound like a person. The "Adding soul" list in the Quick pass section says how.
...
**Rule (house-strengthened, see House overrides):** The final rewrite must not contain em dashes (U+2014) or en dashes (U+2013) used as punctuation, and no double hyphens used as dashes, even when the writer's sample uses them. Replace each dash with a period, comma, colon, or parentheses, or rewrite the sentence. Leave hyphens inside code blocks, inline code, commands, paths, and URLs alone.
**Problem:** A dash lets the writer skip choosing how two clauses relate, so a model reaches for it everywhere. Upstream treats one dash as *weak alone*; the house rule treats any dash as a defect because the global writing rule bans them everywhere.
**Before** (the dashes are written out here so this file stays dash-free):
> The new policy [em dash] announced without warning [em dash] affects thousands of workers. The changes [double hyphen] long overdue according to critics [double hyphen] will take effect immediately.
**After:**
...
**Problem:** Models use these words far more often than people do, especially in groups. This is the only vocabulary list in the upstream skill; the House section adds two more (plain-word swaps and abstract metaphor nouns). A formal word outside these lists is not a tell by itself.
**Before:**
...
**Problem:** Words are bolded without a reason, and vertical lists give every item a bold label and a colon. Remove the bold. Turn a labeled list into prose when the labels carry no information of their own. A bold lead-in that ends in a period, names the item, and is followed by genuinely new detail ("**Schema in TypeScript.** Tables live in one file.") is fine, not a tell. Flag separately a label that restates its own line ("**Downgrades:** Downgrades count."): the label and the sentence say the same thing, so keep one of them.
**Before:**
...
**Watch for:** I hope this helps, Of course!, Certainly!, Great question!, You're absolutely right, Would you like..., Want me to...?, Should I continue?, let me know, here is a..., Found the smoking gun!
...
Five patterns ported from pstack's unslop skill on 2026-08-19 (numbered 30 to 34 in humanizer 2.6.0), plus three overrides. Everything here wins over the sections above and over a writing sample.

### H1. Colons as mid-sentence connectors (was 30)

**Problem:** Colons are fine before a list or an example, and fine when they deliver the answer the sentence set up ("definitions only ever drift in one direction: up"). LLMs also use them to weld two full clauses together as a rhetorical setup, which reads as crutch punctuation. The tell is a clause on each side, not a colon followed by a short answer.
**Before:**
> If you're used to cron jobs: instead of writing schedules, you describe conditions in plain English.
**After:**
> You describe conditions in plain English instead of writing cron schedules.

### H2. Abstract metaphor nouns (was 31)

**Watch for:** substrate, wedge, vector, locus, nexus, vantage, primitive (as noun), harness (as metaphor), surface (as in "API surface"), bedrock, scaffolding (as metaphor), modality, paradigm, north star, flywheel, ratchet (as metaphor), gold-plating, endgame, evacuate (for moving code)
**Problem:** These read as technical but usually hide a plainer concrete word. "Substrate" is "base", "wedge in" is "add", "vector" is "way", "gold-plating" is "more than the job needs", "ratchet" is the mechanism's real name or "a limit that only tightens", "endgame" is "the last phase".
**Before:**
> Our content flywheel is the wedge into the enterprise segment, and the community is the substrate it runs on.
**After:**
> Weekly posts bring in enterprise leads, and the community keeps them around.

### H3. Naming a feeling instead of the mechanism (was 32)

**Problem:** Phrases like "SQL you can read", "the database stays close at hand", or "types that follow your schema" describe a feeling, not a fact. Name the mechanism or a number instead. Two tests: can the sentence be restated as a concrete instruction, fact, or number? And could it appear unchanged in another project's docs? If it fails either test, rewrite it or cut it.
**Before:**
> The types follow your schema, so refactoring feels safe.
**After:**
> Rename a column and the build fails until every query that used it is updated.

### H4. Adverbs propping up weak verbs (was 33)

**Problem:** An adverb next to a weak verb usually means the verb is wrong or a number is missing. "Significantly improves" hides the measured delta. "Runs quickly" is "is fast" or the number.
**Before:**
> The new pipeline runs significantly faster and dramatically reduces costs.
**After:**
> The new pipeline cut the nightly run from 40 minutes to 12 and saves about $800 a month.

### H5. Prefer the plain word (was 34)

utilize becomes use; leverage becomes use; facilitate becomes help; numerous becomes many; commence becomes start; prior to becomes before; in the event that becomes if; in order to becomes to; due to the fact that becomes because; it is important to note that gets deleted. The fancier synonym is rarely clearer.

### House overrides

- **Em dashes and double dashes are banned everywhere, always.** Not in the final rewrite, not when the writer's sample uses them, not in commit messages, code comments, Jira text or emails. This strengthens section 8, where upstream allows a dash when the sample has one. Rewrite with commas, periods, colons before a list, or parentheses.
- **Parentheses are allowed.** The original pstack unslop rule also banned parentheses as a dash substitute. The global rule explicitly permits commas, periods, or parentheses, so a parenthetical aside is never a tell on its own.
- **Romanian text is written in ASCII, without diacritics,** in every deliverable.
- **The humanizer pass is the mandatory final gate** on any prose deliverable per the global CLAUDE.md. The Quick pass below is the in-flight cleanup, not a replacement for the full process above.

## Quick pass

The short list for a fast cleanup (say "unslop" or "quick pass"). Same rules as above, one line each, numbered for reference in a review comment. Run the four steps of "How to work" in miniature: mark, rewrite, add soul, self-audit.
...
2. Name-dropping: media outlets listed without context. Pick one, say what was said.
3. Superficial -ing riders: "highlighting...", "ensuring...", "reflecting...", "showcasing...". Delete or expand with a real source.
4. Promotional language: "nestled", "vibrant", "breathtaking", "groundbreaking", "renowned", "stunning". Use neutral descriptions.
5. Vague attributions: "Experts believe", "Industry reports suggest". Name the source or delete.
6. Formulaic challenges: "Despite challenges... continues to thrive." Replace with specific facts.
7. AI vocabulary: additionally, crucial, delve, enduring, enhance, fostering, garner, interplay, intricate, landscape, pivotal, showcase, tapestry, testament, underscore, vibrant, robust, meticulous. Plain words instead.
...
11. Synonym cycling: pick one word for the thing and repeat it.
12. False ranges: "from X to Y" where X and Y are not on a scale. List the topics.
13. Em dashes and double hyphens: banned outright. Commas, periods, colons before a list, or parentheses.
14. Colons as mid-sentence connectors: rewrite so the point stands without the setup.
15. Bold overuse: do not bold every proper noun or acronym.
16. Inline-header lists: a bold label plus colon that restates the line becomes prose. A bold lead-in that ends in a period and is followed by new detail is fine.
...
25. Generic conclusions: "The future looks bright." State a plan or a fact, or end on the last fact.
26. Abstract metaphor nouns: substrate, wedge, vector, nexus, bedrock, scaffolding, paradigm, north star, flywheel, gold-plating, endgame. Pick the concrete word.
27. Say what it does, not how it feels: name the mechanism or the number; cut a sentence that could sit unchanged in another project's docs.
28. Dense sentences: split when the reader has to backtrack. One idea per sentence.
29. Active voice: "queries are validated" becomes "the compiler validates queries". Passive only when the actor is unknown or does not matter.
30. Adverbs on weak verbs: "significantly improves" becomes the measured delta.
31. Plain words: utilize, leverage, facilitate, numerous, commence, prior to all have a shorter equivalent.
32. One-line closers and dramatic fragments: cut the line that restates the paragraph; merge fragment rows into a sentence with a claim.
33. Staged openers: "Let's dive in", "Here's the thing", "Honestly?" before a routine claim. Remove the run-up.
34. Arguing with no one: "I'm not saying...", "To be clear...", "A tempting approach would be..." with no objection in sight. Cut or state the claim.

## Source

Sections 1 to 25: Wikipedia's ["Signs of AI writing"](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing), maintained by WikiProject AI Cleanup, as organised in [blader/humanizer](https://github.com/blader/humanizer) 3.0.0. House patterns H1 to H5 and Quick pass rules 26 to 31: pstack's unslop skill by poteto (`cursor/plugins/pstack/skills/unslop`, local port of 2026-08-19; the upstream path returned 404 on 2026-09-13).
